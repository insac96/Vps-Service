...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { _id, category, name, description, images, price } = body
    if(!_id || !category || !name || !description || !price) throw 'Dữ liệu đầu vào không hợp lệ'
    if(!Array.isArray(images)) throw 'Dữ liệu hình ảnh không hợp lệ'
    if(!price || isNaN(Number(price))) throw 'Giá sản phẩm không hợp lệ'
